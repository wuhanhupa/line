...
<div class="autobox">
	<div class="now">
    	<a href="/" class="orange">首页</a> &gt;<a href="<?php echo U('Article/index',array('id'=>$shang['id'],'type'=>1));?>" class="orange"> <?php echo ($ArticleTypeList[$data['type']]['title']); ?> </a> &gt; <?php echo ($data["title"]); ?>
  	</div>
	<div class="assets_center clear po_re zin70">
		<!--左侧菜单-->
		<div class="coin_menu">
			<div class="coin_menu_box">
        		<?php if(is_array($ArticleTypeList)): $index = 0; $__LIST__ = $ArticleTypeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($index % 2 );++$index;?><dl>
                    <dt <?php if(($data['type']) == $key): ?>class="here"<?php endif; ?> >
                        <?php if($index == 1): ?><i class="ui-icon ui-icon-tongzhi"></i>
                        <?php elseif($index == 2): ?>
                            <i class="ui-icon ui-icon-hangyezixun"></i>
                        <?php elseif($index == 3): ?>
                            <i class="ui-icon ui-icon-changjianwenti"></i><?php endif; ?>
						<a href="<?php echo U('Article/index',array('id'=>$vo['id'],'type'=>1));?>"><?php echo ($vo["title"]); ?></a>
					</dt>
    	...
    	<div class="assets_content w900 right bg_w mh1500">
      		<h1 class="h1news"><?php echo ($data["title"]); ?></h1>
      		<div class="new_list">
        		<div class="rightcontent">
					<div class="articletime">
						<span>发布时间：<?php echo (date("Y-m-d H:i:s",$data["addtime"])); ?></span>
						<span>发布人：<?php echo C('web_title');?></span>
					</div>
