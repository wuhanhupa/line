                ...
                <div class="form-box trade-main clearfix">
                            ...
                            <table class="dealbox">
                                ...
                                <tr>
                                    <td class="input-td">
                                        ...
                                        <input id="buy_amount" type="text" maxlength="10" placeholder="0"
                                               class="inputRate" v-model="buyPrice">
                                    </td>
                                </tr>
                                <tr>
                                    <td class="input-td">
                                        <span class="b-unit input-title">买入量 USDT</span>
                                        <input id="buy_vol" type="text" maxlength="10"  placeholder="0"
                                               class="inputRate" v-model="buyNum">
                                    </td>
                                </tr>
                                <tr>
                                    <td class="input-td">
                                ...
                                <tr>
                                    <td class="input-td" style="border:0">
                                        <?php if($buy == true ): ?><input type="button" class="btnAskBid jiaoyi_btn  button button-flat-action"
                                                   t="ask" id="buy_submit" value="买入 (CNY→USDT)">
                                            <?php else: ?>
                                            <input type="button" class="btnAskBid jiaoyi_btn  button button-flat-action"
                                                   t="ask" id="buy_submit" disabled="disabled" value="买入 (CNY→USDT)"><?php endif; ?>
                                    </td>
                                </tr>
                                </tbody>
                            </table>
                    ...
                    <div class="mairu mairu-form">
                        <div class="m_title">
                            ...
                            <span class="yu-e">
                                    <span>(</span>
                                    <span>USDT可用余额:</span>
                                    <span id="balance_sell_able" class="green"><?php echo ($userCoin_top['cny']*1); ?></span>
                                    <span>)</span>
                                </span>
                            ...
                            <table class="dealbox" cellspacing="0">
                                ...
                                <tr>
                                    <td class="input-td">
                                        ...
                                        <input id="sell_rate" class="inputRate green" maxlength="10" readonly="readonly"
                                               :value="sellRate">
                                    </td>
                                </tr>
                                <tr>
                                    <td class="input-td">
                                        ...
                                        <input id="sell_amount" class="inputRate" maxlength="10" v-model="sellPrice"  placeholder="0">
                                    </td>
                                </tr>
                                <tr>
                                    <td class="input-td">
                                        <span class="b-unit input-title">卖出量 USDT</span>
                                        <input id="sell_vol" class="inputRate" maxlength="10" placeholder="0"
                                               v-model="sellNum">
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <td class="input-td" style="border:0">
                                        <?php if($sell == true ): ?><input type="button" class="btnAskBid jiaoyi_btn  button button-flat-action"
                                                   t="bid" id="sell_submit" value="卖出 (USDT→CNY)">
                                            <?php else: ?>
                                            <input type="button" class="btnAskBid jiaoyi_btn  button button-flat-action"
                                                   t="bid" id="sell_submit" disabled="disabled" value="卖出 (USDT→CNY)"><?php endif; ?>
                                    </td>
                                </tr>
                                </tbody>
                            </table>
                    ...
                    <div class="clearfix">
                        ...
                        <div class="c2c-notice-text-box ">
                            ...
                            <p>3.请使用本人绑定的银行卡、支付宝付款,暂不支持微信支付</p>
                            <p>4.商家全天候处理订单，一般接单后24小时会完成打款</p>
                            <p class="color-red">5.工作日9:00-17:00单笔订单不得超过￥100000.00，其他时间单笔订单不得超过￥50000.00</p>
                            <p>6.切勿备注比特币等虚拟货币信息</p>
                            <p class="color-red">7.支付过程中出现异常时（未完成支付），请重新下单</p>
                            <p>8.实际支出/收入金额以第三方为准，平台只做参考</p>
    ...
    <div>
        <div class="orders-nav">
            <ul>
                ...
                <li  class="active" name="comp">已完成订单</li>
        ...
        <div class="order-list-header Cztrade-order-list-header">
            <ul>
                ...
                <li>金额(CNY)</li>
                <li>状态</li>
                <li style="width:15%;">建立时间</li>
                <li v-if="type < 2">操作</li>
            </ul>
        </div>
        <div class="list-box Cztrade-list-box">
            <ul class="g-scrollbar">
                <li class="order-item " v-for="(item,index) in result" :tradeId="item.trade_id" :buyId="item.buyid"
                    :sellId="item.sell_id" :num="item.num" :type="item.type" :id="item.userid">
                    <div class="order-info">
                        <span style="width:25%;">{{ item.order_no }}</span>
                        <span>{{ item.type > 1 ? '卖出': '买入' }}</span>
                        <!--<span>{{ item.parities ? item.parities : '--' }}</span>-->
                        <span>{{ item.num }}</span>
                        <span>{{ item.price }}</span>
                        <span>{{ item.status }}</span>
                        <span style="width:15%;">{{ item.ctime }}</span>
    ...
    Vue.component('app-component', {
        ...
        watch: {
            buyNum: function (curVal, oldVal) { // 监听下单买入数量
                ...
                if (vm.buyNum_on && !vm.buyPrice_on) {
                    vm.buyNum = vm.NumberCheck(curVal,2);
            ...
            buyPrice: function (curVal, oldVal) { // 监听买入金额
                var vm = this;
                if (!vm.buyNum_on && vm.buyPrice_on) {
                    vm.buyPrice = vm.NumberCheck(curVal,2);
                    vm.buyPrice < 1 ? vm.buyPrice = '' : '';
                    vm.buyNum = Math.floor(parseFloat((vm.buyPrice / vm.buyRate) * 100)) / 100;
                }
            },
            sellNum: function (curVal, oldVal) { // 监听卖出数量
                var vm = this;
                if (vm.sellNum_on && !vm.sellPrice_on) {
                    vm.sellNum = vm.NumberCheck(curVal,2);
            ...
            sellPrice: function (curVal, oldVal) { // 监听卖出金额
                var vm = this;
                if (!vm.sellNum_on && vm.sellPrice_on) {
                    vm.sellPrice = vm.NumberCheck(curVal,2);
                    vm.sellPrice < 1 ? vm.sellPrice = '' : '';
                    vm.sellNum = Math.floor(parseFloat((vm.sellPrice / vm.sellRate) * 100)) / 100;
                }
            },
        ...
        methods: {
            filerInput: function (val) {
                ...
                return str.replace(/^0[0-9]/, '').replace(/\.+/g, '').replace(/[a-zA-Z]|\+|\-/g, '').replace(
                    /[\u4e00-\u9fa5]/g, '');
