...
<script>
	function SendCode(){
		...
		$.post("<?php echo U('Verify/findpwd');?>",{username:username,moble:moble,verify:verify},function(data){
			if(data.status==1){
	...
	function Update(){
		...
		$.post("<?php echo U('Login/findpwd');?>",{username:username,moble:moble,moble_verify:moble_verify,verify:verify,password:password,repassword:repassword},function(data){
			if(data.status==1){
				layer.msg(data.info,{icon:1});
				setTimeout(function(){
					window.location = '/';
