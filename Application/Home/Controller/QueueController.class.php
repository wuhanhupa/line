...
class QueueController extends HomeController
{
    ...
    public function upTrade($market = null)
    {
        ...
        $sunfa = new \Common\Design\Suanfa\SnowFlake();
        $id = $sunfa->nextId();
        ...
        if ($type == 1) {
            ...
            $rs[] = $finance_nameid = $mo->table('btchanges_trade')->add(array('id' => $id, 'userid' => $userid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 1, 'addtime' => time(), 'status' => 0));
            ...
            $rs[] = $mo->table('btchanges_trade')->add(array('id' => $id, 'userid' => $userid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 2, 'addtime' => time(), 'status' => 0));
        } else {
            $mo->execute('rollback');
            $mo->execute('unlock tables');
            return '交易类型错误';
