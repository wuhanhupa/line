...
//富pay支付
class FupayController extends CommonController
{
    //商户ID
    ...
    //提交订单
    public function createOrder($jmcode)
    {
        ...
        if ($result['result']) {
            ...
            if ($add && $type == 1) {
                ...
                $request = [
                    ...
                    'pickupUrl' => $this->callback_url . '/Papi/Fupay/notice', //  交易完成后跳转URL
                    'signType' => 'MD5', //固定值不要改
                ];
    ...
    //订单列表
    public function orderList($jmcode)
    {
        $result = $this->rests($jmcode);
        ...
        if ($result['result']) {
            ...
            foreach ($list as $k => $v) {
                ...
                $data[$k]['order_no'] = (string)$v['order_no'];
                $data[$k]['num'] = (string)sprintf("%.2f", $v['num']);
                $data[$k]['type'] = (string)$v['type'];
                $data[$k]['price'] = (string)round($v['price'], 2);
                $data[$k]['status'] = (string)$arr[$v['status']];
                $data[$k]['ctime'] = (string)date('Y-m-d H:i:s', $v['ctime']);
            }
