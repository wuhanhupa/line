...
{
    public function index($name = null, $field = null, $status = null, $ident_status = null)
    {
        //$this->checkUpdata();
        //$where = array();
        $where = ' 1 ';
        ...
        if ($ident_status != null) {
            $where = $where.' and `ident_status` = '.$ident_status;
        }
    ...
    {
        $uid = $_POST['uid'];
        //判断当前用户是否已经审核了如果审核已经通过不能用在审核
        $ident_status = M('User')->where(array('id' => $uid))->getField('ident_status');
        if ($ident_status == 2) {
            $info['status'] = '2002';
            $info['info'] = '当前用户已经审核通过';

            $this->ajaxReturn($info);
        }
        $save = M('User')->where(array('id' => $uid))->save(array('ident_status' => $_POST['ident_status']));
        if ($save) {
            $info['status'] = '0000';
            $info['info'] = '操作成功！';
    ...
    public function coinEdit($id = null)
    {
        if (empty($_POST)) {
            ...
            if ($id) {
                $old = M('UserCoin')->where(array('id' => trim($id)))->find();

                if (M('UserCoin')->save($_POST)) {
                    //添加系统日志
                    $title = '管理员'.session('admin_username').'修改用户ID为'.$old['userid'].'的资产';
                    $content = '';
                    foreach($_POST as $k => $v) {
                        if ($k != 'id') {
                            if ($old[$k] != $v) {
                                $content .= $k.'资产从'.$old[$k].'到'.$v.';'.'<br>';
                            }
                        }
                    }
                    M('Logs')->add(array(
