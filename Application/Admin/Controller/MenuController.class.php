...
class MenuController extends AdminController
{
    ...
    {
        $this->editRow('Menu', ['hide' => $value], ['id' => $id]);
    }

    public function toogleDev($id, $value = 1)
    {
        $this->editRow('Menu', ['is_dev' => $value], ['id' => $id]);
    }
    ...
    public function importFile($tree = NULL, $pid = 0)
    {
        ...
        foreach ($tree as $value) {
            $add_pid = $menuModel->add(['title' => $value['title'], 'url' => $value['url'], 'pid' => $pid, 'hide' => isset($value['hide']) ? (int)$value['hide'] : 0, 'tip' => isset($value['tip']) ? $value['tip'] : '', 'group' => $value['group']]);
        ...
        if (IS_POST) {
            ...
            if ($lists == []) {
                ...
                foreach ($lists as $key => $value) {
                    ...
                    if (count($record) == 2) {
                        $menuModel->add(['title' => $record[0], 'url' => $record[1], 'pid' => $pid, 'sort' => 0, 'hide' => 0, 'tip' => '', 'is_dev' => 0, 'group' => '']);
                    }
                }
    ...
    public function sort()
    {
        if (IS_GET) {
            ...
            foreach ($ids as $key => $value) {
                $res = M('Menu')->where(['id' => $value])->setField('sort', $key + 1);
            }
